...
action(move(Agent,From,To)):-ground_robot(Agent),place(From),place(To).
action(fly(Agent,From,To)):-flying_robot(Agent),place(From),place(To).
...
% Causes formulae 
causes(move(Agent,From,To),in(Agent,To),[]):-	ground_robot(Agent),place(From),place(To).
causes(move(Agent,From,To),mneg(in(Agent,From)),[]):-	ground_robot(Agent),place(From),place(To).

causes(fly(Agent,From,To),in(Agent,To),[]):-	flying_robot(Agent),place(From),place(To).
causes(fly(Agent,From,To),mneg(in(Agent,From)),[]):-	flying_robot(Agent),place(From),place(To).

% Caused formulae
caused([in(Agent,To)],mneg(in(Agent,From))):- (ground_robot(Agent);flying_robot(Agent)),place(From),place(To).
caused([in(Agent,To),in(Victim,To)],saved(Victim)):- (ground_robot(Agent);flying_robot(Agent)),victim(Victim),place(To).

% Action precondition
executable(move(Agent,From,To),[in(Agent,From)]):- ground_robot(Agent),place(From),place(To),path(Agent,From,To).
executable(fly(Agent,From,To),[in(Agent,From)]):- flying_robot(Agent),place(From),place(To),path(Agent,From,To).
